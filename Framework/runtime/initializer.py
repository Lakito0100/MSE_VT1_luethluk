...
def init_fields(cfg, st, gs):
    """Initialize state fields for frost, edge, fin, and tube domains."""

    # Initialize frost thickness
    st.s_e = np.zeros(gs.ntheta, dtype=float)
    st.s_e[:] = 1.0e-6
    st.s_ft = 1.0e-6

    # Initialize edge domain [r, theta]
    st.T_e = np.zeros((gs.nr, gs.ntheta), dtype=float)
    st.T_e[:, :] = cfg.T_tube
    st.rho_e = np.zeros((gs.nr, gs.ntheta), dtype=float)
    st.rho_e[:, :] = 25.0  # Initial frost density
